...
-- 명시적 형변환
-- CAST (experssion as 데이터형식[(길이)])
-- convert (experssion, 데이터형식[(길이)])
-- 데이터 형식으로 가능한 것은 binary, char, date, decimal, json
-- signed integer, time, unsinged integer 등이 있다.
...
select 
    ...
    -- singed integer : 음수, 0, 양수 모두 포함하는 정수 값을 저장할 수 있다.
